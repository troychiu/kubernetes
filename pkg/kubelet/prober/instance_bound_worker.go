...
func (w *instanceBoundWorker) firstProbeDelay() time.Duration {
	...
	delay := time.Until(w.startedAt.Add(time.Duration(w.spec.InitialDelaySeconds) * time.Second))
	if delay < 0 {
		delay = 0
	}
	if w.adopted {
...
func (w *instanceBoundWorker) doProbe() (keepGoing bool) {
	...
	case w.probeType == liveness && result == results.Failure:
		// The sync loop will kill the container. Nothing useful can be learned
		// by probing it further, and probing a container that is being torn
		// down is what #21751 is about. If it comes back, it comes back with
		// new workers.
