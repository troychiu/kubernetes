...
// StopLivenessAndStartup stops the pod's liveness and startup workers. Killing
// a container stops its own probes, which covers the cases this used to; it is
// kept as a cheap, idempotent backstop for a pod that is beginning to
// terminate.
...
func (m *instanceBoundManager) removePodWorkers(match func(types.UID) bool) {
	...
	for key, w := range m.workers {
		...
		m.stopWorkerLocked(key, w)
		// The metric label sets are pod- and container-name-scoped, so this --
		// the pod going away -- is the point at which they stop being useful.
		w.deleteMetrics()
		stale.Insert(w.containerID)
	}
	for _, id := range stale.UnsortedList() {
