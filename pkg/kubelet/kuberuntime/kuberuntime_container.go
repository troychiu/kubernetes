...
func (m *kubeGenericRuntimeManager) killContainer(ctx context.Context, pod *v1.Pod, containerID kubecontainer.ContainerID, containerName string, message string, reason containerKillReason, gracePeriodOverride *int64, ordering *terminationOrdering) error {
	...
	// already being killed, so nothing is gained by probing it further, and an
	// exec probe running inside a container that is being torn down is what
	// #21751 is about. Readiness keeps going, so that a container draining
	// through its PreStop hook and grace period is taken out of service.
